...
public class change_budget_e extends JFrame implements ActionListener{
	...
	public change_budget_e(User u) throws IOException {
		this.u = u;
		this.setSize(1000,800);
		this.setTitle("change budget (electricity)");
	...
	private void initFrame() throws IOException{
		...
		textp3.setText(u.read_budget("e"));		//current elec budget				
		...
		public void actionPerformed(ActionEvent e) {
			if(e.getSource() == change_budget_e.button1){    //OK
				String budget = text1.getText();
				try {
					u.set_budget("e", budget);
			...
			else if(e.getSource() == change_budget_e.button2){   //go back
				this.dispose();
				try {
					new uhome(u.get_id());
			...
			else if(e.getSource() == change_budget_e.button3){   //home
				this.dispose();
				try {
					new uhome(u.get_id());
				} catch (FileNotFoundException e1) {
...
//		change_budget_e ce = new change_budget_e();
//		
//	}
